...
public class ZipFS extends VFSStub {
    ...
    public ZipFS(byte[] data) throws IOException {
        BufferedInputStream bis = new BufferedInputStream(new ByteArrayInputStream(data)); bis.mark(data.length);
        ZipInputStream zs = new ZipInputStream(bis); byte[] buffer = new byte[2048];
        for(ZipEntry e; ( e = zs.getNextEntry() ) != null; ) {
            ...
            } String[] split = DIRECTORYSPLIT.split(e.getName()); SimpleVFile dir = this;
            for(int i = 0; i < ( split.length - 1 ); i++) {
                String dirName = split[i]; SimpleVFile sub = dir.get(dirName); if(sub == null) {
                    sub = new MockFile(dirName); dir.add(sub);
                } dir = sub;
